"""Tests for Flask routes and request validation."""
...
class TestUiRoutes:
    def test_ui_pages_render(self):
        ...
        for path in ["/", "/dashboard", "/orchestrator"]:
            response = client.get(path)
            assert response.status_code == 200
            assert b"BeermannCode" in response.data

    def test_studio_overview_includes_workflows_and_languages(self):
        response = make_client().get("/api/studio/overview")

        payload = response.get_json()
        assert response.status_code == 200
        assert payload["service"] == "BeermannCode"
        assert {workflow["id"] for workflow in payload["workflows"]} == {"analyze", "review", "generate"}
